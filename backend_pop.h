...
// struct pop_selection( pop_map *map, int gender, double age_low, double age_high, bool random=true);
...
struct pop_model
{
  ...
  int init_generations()
  {
    ...
    {
      case 0: return 0;
      case 1: return int( ceil( BLL->max_life ) );
    }
  }
  ...
  double const_pop_fert(double n) //equilibrium fertility rate for pop of size n.
  {
    ...
    {
      case 0: return 0.0;
      case 1: return BLL->const_pop_fert(n);
    }
  }
  double age_of_death() //produce a random death age for a new person.
  {
    switch (model_type)
    {
      case 0: return -1.0; //no automatic death
      case 1: return BLL->age_of_death();
    }
  }
...
{
  pop_map *map;
  int gender; //-1 : both, 1: female, 0: male
  double age_low;
  double age_high;
  ...
  pop_selection( pop_map *map, int gender, double age_low, double age_high, bool random=true)
    : map(map), gender(gender), age_low(age_low), age_high(age_high)
    {
      double birth_low = age_high > 0 ? map->t_now - age_high : age_high;
      double birth_high = age_low > 0 ? map->t_now - age_low : age_low;
      if (gender < 1) //add males
      {
      ...
      //randomise
      if (true == random){
        std::sort( selection.begin(),  selection.end(), [](auto const &A, auto const &B ){return A.first < B.first; } ); //sort only by rnd
      }
    }
};
...
struct pop_person
{
  ...
  //constructor
  pop_person(int uID, int f_uID, int m_uID, bool female, double t_birth, double age, double d_age) :
    alive(true), uID(uID), f_uID(f_uID), m_uID(m_uID), female(female), t_birth(t_birth), age(age), d_age(d_age)
  { }
};
